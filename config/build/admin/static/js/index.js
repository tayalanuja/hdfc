...
$(document).ready(function() {
  ...
  const success = {
    'url': 'https://credit.in-d.ai/hdfc_life/credit/admin_dashboard',
    'headers': {
    ...
    if (xhr.status === 200) {
      ...
      $('#dahsobard_table').DataTable({
        ...
        'columns': [
          ...
          // {'data': 'review_status', 'class': 'review_status'},
          {'data': 'job_id',
          'searchable': false,
          'render': function(data, type, row) {
            // return `<a href="${row.review_status == 'No'? 'localization.html?job_id=data:'sdadasd'} 'class="btn Complete">Procceed '+JSON.stringify(row.review_status)+' </a>`;
            return `<a class="btn btn-secondary" href='${row.review_status == 'No'? 'localization.html?job_id='+data+'':'calculations.html?job_id='+data+''}'>${row.review_status}</a>`
          },
        },
          ...
          {'data': 'job_id',
            ...
            'render': function(data, type, row) {
              // return `<a href="${row.review_status == 'No'? 'localization.html?job_id=data:'sdadasd'} 'class="btn Complete">Procceed '+JSON.stringify(row.review_status)+' </a>`;
              return `<a class="btn Complete" href='${row.review_status == 'No'? 'localization.html?job_id='+data+'':'localization.html?job_id='+data+''}'>Procceed</a>`
            },
          },
          ...
          // "searchable": false,
          // "render": function (data, type, row) {
          //     return '<a onClick="submit_flag(this)" class="btn Complete">Submit Batch</a>';
