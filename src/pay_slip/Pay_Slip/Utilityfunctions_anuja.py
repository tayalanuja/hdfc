...
########### EmployerName Extraction ########################################################
    try:
        ...
        if employername_cords:
            ...
            # employernamedf_org=df.copy()
            # print(linewise_df)
            comp_id=['ltd','limited','inc','co','pvt','private','solution','solutions','consultancy','services']
            wordlist=[".","%","+",",","/"]
            ...
            for word in words:
                if employername_text=='NA' and word not in wordlist and (word.lower() in comp_id or TextBlob(word.lower()).words[0].spellcheck()[0][0] in comp_id):
                    if words.index(word) >3:
                        ...
                        employername_text=' '.join(company)
                        confidence_employername=70
            if employername_text=='':
                employername_text= "NA"
                confidence_employername=0
    except Exception as e:
        employername_text= traceback.format_exc()
        confidence_employername=0
    print(confidence_employername,employername_text)
...
########### EmployeeName Extraction ########################################################
    try:
        ...
        if employeename_cords:
            ...
            employeename_index = linewise_df[linewise_df['Token'].str.contains('name|emp name|employee name|empname|employeename|emp. name|emp name.:|employee name|employee name.:|emp name:|employee name:|name of the employee', na=False, case = False, regex = True)].index.tolist()
            if employeename_index:
                ...
                employeenamedf = employeenamedf.astype(str)
                print(employeenamedf)
                ...
                employeenamedf = employeenamedf[employeenamedf['Token'].str.contains('mr|ms|name|name.:|name:', na=False, case = False, regex = True).shift(1).fillna(False)]
        ...
        employeename_text= traceback.format_exc()
        ...
        if periodofpay_cords:
            ...
            if periodofpaydf.empty:
                ...
                if periodofpaydf.empty:
                    ...
                    periodofpay_list = periodofpaydf[0].tolist()
            ...
            periodofpay_index = linewise_df[linewise_df['Token'].str.contains('month|period|pay slip', na=False, case = False, regex = True)].index.tolist()
...
########### Net Salary Extraction ########################################################
    try:
        ...
        if netsalary_cords:
            ...
            #netsalarydf['Token'] = netsalarydf['Token'].str.replace(",|:| ","")
            netsalary_index = linewise_df[linewise_df['Token'].str.contains('net salary|base salary|netsalary|take home|credited:|net pay|nett pay|net amount', na=False, case = False, regex = True)].index.tolist()
            if netsalary_index:
        ...
        #print(traceback.print_exc())
        netsalary_text= traceback.format_exc()
        confidence_netsalary=0
